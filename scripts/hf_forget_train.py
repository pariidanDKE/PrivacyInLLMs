...
def main(configs):
    ...
    output_dir = HydraConfig.get().runtime.output_dir # DP : HERE IS WHERE WE FETCH OUTPUT_DIR FROM ENVIRONMENT VARS
    ...
    all_choices = OmegaConf.to_container(HydraConfig.get().runtime.choices) # DP: THIS IS WHERE THE RUNTIME ARGS GET EXTRACTED
    configs.name = "|".join([
